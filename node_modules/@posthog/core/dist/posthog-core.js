...
class PostHogCore extends external_posthog_core_stateless_js_namespaceObject.PostHogCoreStateless {
    ...
    reset(propertiesToKeep) {
        this.wrap(()=>{
            ...
            if (this.disableRemoteFeatureFlags && !this.disabled) {
                if (!allPropertiesToKeep.includes(external_types_js_namespaceObject.PostHogPersistedProperty.FeatureFlagDetails)) this.setKnownFeatureFlagDetails({
                    flags: {}
                });
            } else this.reloadFeatureFlags();
        });
    }
    ...
    async flagsAsync(options) {
        ...
        await this._initPromise;
        if (this.disabled) return;
        if (this.disableRemoteFeatureFlags && !fetchConfig) return void this._logger.info('Feature flags are disabled (disableRemoteFeatureFlags), skipping reload.');
            ...
            return super.getRemoteConfig().then((response)=>{
                if (response) {
                    ...
                    if (false === response.hasFeatureFlags) {
                        if (!this.disableRemoteFeatureFlags) this.setKnownFeatureFlagDetails({
                            flags: {}
                        });
                        this._logger.warn('Remote config has no feature flags, will not load feature flags.');
                    } else if (false !== this.preloadFeatureFlags && !this.disableRemoteFeatureFlags) {
                        willLoadFlags = true;
                        this.flagsAsync({
    ...
    async _flagsAsync(options) {
        ...
        this._flagsResponsePromise = this._initPromise.then(async ()=>{
            ...
            const deviceId = this.getPersistedProperty(external_types_js_namespaceObject.PostHogPersistedProperty.DeviceId);
            const extraProperties = {
                $anon_distinct_id: sendAnonDistinctId ? this.getAnonymousId() : void 0,
                $device_id: deviceId ?? void 0,
                ...this.disableRemoteFeatureFlags ? {
                    disable_flags: true
                } : {}
            };
    ...
    getAllFeatureFlags() {
        return (0, external_featureFlagUtils_js_namespaceObject.flagDetailsToResults)(this.getFeatureFlagDetails()?.flags ?? {});
    }
    _getFeatureFlagResult(key, options = {}) {
        ...
        if (sendEvent) {
            ...
            const properties = {
                ...
                ...(0, external_posthog_core_stateless_js_namespaceObject.maybeAdd)('$feature_flag_has_experiment', featureFlag?.metadata?.has_experiment)
            };
    ...
    reloadFeatureFlags(options) {
        if (this.disableRemoteFeatureFlags && !this.disabled) return void this._initPromise.then(()=>options?.cb?.(void 0, this.getFeatureFlags()));
        this.flagsAsync({
            sendAnonDistinctId: true
        }).then((res)=>{
    ...
    updateFlags(flags, payloads, options) {
        this.wrap(()=>{
            const existingDetails = options?.merge ? this.getKnownFeatureFlagDetails()?.flags : void 0;
            const existingFlags = existingDetails ? (0, external_featureFlagUtils_js_namespaceObject.getFlagValuesFromFlags)(existingDetails) : {};
            const existingPayloads = {};
            for(const key in existingDetails){
                const storedPayload = existingDetails[key].metadata?.payload;
                if (void 0 !== storedPayload) existingPayloads[key] = (0, external_featureFlagUtils_js_namespaceObject.parsePayload)(storedPayload);
            }
            const finalFlags = {
                ...existingFlags,
                ...flags
            ...
            for (const [key, value] of Object.entries(finalFlags)){
                ...
                flagDetails[key] = {
                    key,
                    enabled: (0, external_featureFlagUtils_js_namespaceObject.getEnabledFromValue)(value),
                    variant: (0, external_featureFlagUtils_js_namespaceObject.getVariantFromValue)(value),
                    reason: void 0,
                    metadata: {
                        id: void 0,
                        version: void 0,
    ...
    unsetPersonProperties(propertyNames, reloadFeatureFlags = true) {
        this.wrap(()=>{
            const names = ((0, index_js_namespaceObject.isArray)(propertyNames) ? propertyNames : [
                propertyNames
            ]).filter((name)=>(0, index_js_namespaceObject.isString)(name) && name.length > 0);
            if (0 === names.length) return;
            if (!this._requirePersonProcessing('posthog.unsetPersonProperties')) return;
            const existingProperties = this.getPersistedProperty(external_types_js_namespaceObject.PostHogPersistedProperty.PersonProperties) || {};
            const nextProperties = {
                ...existingProperties
            };
            for (const name of names)delete nextProperties[name];
            this.setPersistedProperty(external_types_js_namespaceObject.PostHogPersistedProperty.PersonProperties, nextProperties);
            if (reloadFeatureFlags) this.reloadFeatureFlags();
            this.capture('$set', {
                $unset: names
            });
    ...
    processBeforeEnqueue(message) {
        ...
        return {
            ...message,
            uuid: (0, index_js_namespaceObject.getEventUuid)(result.uuid ?? message.uuid, uuidv7_js_namespaceObject.uuidv7),
            event: result.event,
