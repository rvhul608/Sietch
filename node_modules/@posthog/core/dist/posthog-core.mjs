...
class PostHogCore extends PostHogCoreStateless {
    ...
    reset(propertiesToKeep) {
        this.wrap(()=>{
            ...
            if (this.disableRemoteFeatureFlags && !this.disabled) {
                if (!allPropertiesToKeep.includes(PostHogPersistedProperty.FeatureFlagDetails)) this.setKnownFeatureFlagDetails({
                    flags: {}
                });
    ...
    async _flagsAsync(options) {
        ...
        this._flagsResponsePromise = this._initPromise.then(async ()=>{
            ...
            const deviceId = this.getPersistedProperty(PostHogPersistedProperty.DeviceId);
    ...
    updateFlags(flags, payloads, options) {
        this.wrap(()=>{
            ...
            for(const key in existingDetails){
                ...
                if (void 0 !== storedPayload) existingPayloads[key] = parsePayload(storedPayload);
            }
            const finalFlags = {
                ...existingFlags,
                ...flags
    ...
    unsetPersonProperties(propertyNames, reloadFeatureFlags = true) {
        this.wrap(()=>{
            const names = (isArray(propertyNames) ? propertyNames : [
                propertyNames
            ...
            this.setPersistedProperty(PostHogPersistedProperty.PersonProperties, nextProperties);
            if (reloadFeatureFlags) this.reloadFeatureFlags();
            this.capture('$set', {
                $unset: names
            });
