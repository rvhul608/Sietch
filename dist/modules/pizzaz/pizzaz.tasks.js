...
let PizzazTaskTools = class PizzazTaskTools {
    pizzazService;
    constructor(pizzazService) {
        this.pizzazService = pizzazService;
    }
    /**
     * audit_pizza_shops
     *
     ...
     */
    async auditPizzaShops(args, ctx) {
        ...
        for (let i = 0; i < shops.length; i++) {
            ...
            if (ctx.task) {
                ...
                ctx.task.updateProgress(`🔍 Auditing "${shop.name}" (${i + 1}/${shops.length})…`);
            }
            ...
            const notes = [];
            if (score >= 90)
                notes.push('Exceptional rating and review volume.');
            if (!shop.openNow)
                notes.push('Currently closed — may affect customer reach.');
            if (shop.rating >= 4.7)
                notes.push('One of the highest-rated shops.');
            if (shop.reviews < 100)
                notes.push('Low review count — still building reputation.');
            results.push({
                shopId: shop.id,
                shopName: shop.name,
                score,
...
__decorate([
    Tool({
        ...
        description: 'Runs a quality audit across all pizza shops. ' +
...
], PizzazTaskTools.prototype, "auditPizzaShops", null);
__decorate([
    Tool({
        name: 'order_pizza',
        description: 'Places an order at a pizza shop. This tool REQUIRES task augmentation — ' +
    ...
    __metadata("design:paramtypes", [void 0, Object]),
    __metadata("design:returntype", Promise)
], PizzazTaskTools.prototype, "orderPizza", null);
PizzazTaskTools = __decorate([
    Injectable({ deps: [PizzazService] }),
    __metadata("design:paramtypes", [PizzazService])
], PizzazTaskTools);
export { PizzazTaskTools };
//# sourceMappingURL=pizzaz.tasks.js.map